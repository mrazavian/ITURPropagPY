...
from iturpropag.utils import prepare_input_array, prepare_output_array, dataset_dir,\
...
class _ITU836_6():
    ...
    def __init__(self):
        ...
        self.year = 2017
        self.month = 12
        self.link = 'https://www.p.int/rec/R-REC-P.836-6-201712-I/en'
    ...
    def VSCH(self, lat, lon, p):
        if not self._VSCH:
            ...
            d_dir = os.path.join(dataset_dir, 'p836/v6_VSCH_%s.txt')
            lats = load_data(os.path.join(dataset_dir, 'p836/v6_Lat.txt'))
            lons = load_data(os.path.join(dataset_dir, 'p836/v6_Lon.txt'))
            for p_loads in ps:
    ...
    def rho(self, lat, lon, p):
        if not self._rho:
            ps = [0.1, 0.2, 0.3, 0.5, 1, 2, 3, 5, 10, 20, 30,
                  50, 60, 70, 80, 90, 95, 99]
            d_dir = os.path.join(dataset_dir, 'p836/v6_RHO_%s.txt')
            lats = load_data(os.path.join(dataset_dir, 'p836/v6_Lat.txt'))
            lons = load_data(os.path.join(dataset_dir, 'p836/v6_Lon.txt'))
            for p_loads in ps:
                vals = load_data(d_dir % (str(p_loads).replace('.', '')))
...
class _ITU836_5():
    ...
    def __init__(self):
        ...
        self.year = 2013
        self.month = 9
        self.link = 'https://www.p.int/rec/R-REC-P.836-5-201309-I/en'
    ...
    def VSCH(self, lat, lon, p):
        if not self._VSCH:
            ps = [0.1, 0.2, 0.3, 0.5, 1, 2, 3, 5, 10, 20, 30,
                  50, 60, 70, 80, 90, 95, 99]
            d_dir = os.path.join(dataset_dir, 'p836/v5_VSCH_%s.txt')
            lats = load_data(os.path.join(dataset_dir, 'p836/v5_Lat.txt'))
            lons = load_data(os.path.join(dataset_dir, 'p836/v5_Lon.txt'))
            for p_loads in ps:
                vals = load_data(d_dir % (str(p_loads).replace('.', '')))
                self._VSCH[float(p_loads)] = bilinear_2D_interpolator(
                    lats, lons, vals)
    ...
    def rho(self, lat, lon, p):
        if not self._rho:
            ps = [0.1, 0.2, 0.3, 0.5, 1, 2, 3, 5, 10, 20, 30,
                  50, 60, 70, 80, 90, 95, 99]
            d_dir = os.path.join(dataset_dir, 'p836/v5_RHO_%s.txt')
            lats = load_data(os.path.join(dataset_dir, 'p836/v5_Lat.txt'))
            lons = load_data(os.path.join(dataset_dir, 'p836/v5_Lon.txt'))
            for p_loads in ps:
                vals = load_data(d_dir % (str(p_loads).replace('.', '')))
...
class _ITU836_4():
    ...
    def __init__(self):
        ...
        self.year = 2009
        self.month = 10
        self.link = 'https://www.p.int/rec/R-REC-P.836-4-200910-S/en'
    ...
    def rho(self, lat, lon, p):
        if not self._rho:
            ps = [0.1, 0.2, 0.3, 0.5, 1, 2, 3, 5, 10, 20, 30,
                  50, 60, 70, 80, 90, 95, 99]
            d_dir = os.path.join(dataset_dir, 'p836/v4_RHO_%s.txt')
            lats = load_data(os.path.join(dataset_dir, 'p836/v4_Lat.txt'))
            lons = load_data(os.path.join(dataset_dir, 'p836/v4_Lon.txt'))
            for p_loads in ps:
                vals = load_data(d_dir % (str(p_loads).replace('.', '')))
