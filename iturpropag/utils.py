...
def load_data(path, is_text=False, **kwargs):
    """ Loads data files from /iturpropag/data/
    ...
    - **kwargs: 
        Note : for better understanding of **kwargs please refer to 
...
def prepare_input_array(array):
    """ Function "prepare_input_array(array)" Formats an array to be a 2-D numpy-array
...
def prepare_output_array(array, type_input=None):
    ...
    if (type_input in __NUMERIC_TYPES__ and
        ...
        ((isinstance(array, np.ndarray) and array.size == 1) or
         (not type(array) not in __NUMERIC_TYPES__ and np.size(array) == 1))):
        value = float(value)
...
def compute_distance_earth_to_earth(lat_p, lon_p, lat_grid, lon_grid,
                                    method=None):
    '''
    Compute the distance between a point (P) in (lat_p, lon_p) and a matrix of
...
def ccdf(timeseries, bins=10):
    ...
    ind_nan = np.argwhere(np.isnan(timeseries))
    timeseries = np.delete(timeseries, ind_nan)
    ...
    ts_ccdf = 1 - np.cumsum(hist * bin_dx)

    # save the output in structure mode        
    stat = {"pdf" : hist * 100,
            "ccdf" : ts_ccdf * 100,
            "bin_edges" : bin_edges,
            "bin_width" : bin_dx}
